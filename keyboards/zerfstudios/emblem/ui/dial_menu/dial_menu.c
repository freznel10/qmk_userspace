...
void handle_prev_action(menu_obj_t* menu_obj) {
    if(menu_obj->parent != NULL) {
        destroy_objs();
        create_dial_menu(ui_Screen2, menu_obj->parent->parent);
    }
}
...
void handle_next_action(menu_obj_t* menu_obj) {
    if (menu_obj->first_child != NULL) {
        destroy_objs();
        create_dial_menu(ui_Screen2, menu_obj);
    }
}
...
void init_dial_menu(lv_obj_t* panel) {
    ...
    lv_obj_t* ui_Knob = lv_obj_create(panel);

    lv_obj_add_style(ui_Knob, &style_dial_menu_knob, LV_STATE_DEFAULT);

    menu_lbl = lv_label_create(panel);
...
void create_dial_menu(lv_obj_t* panel, menu_obj_t* menu_root) {
